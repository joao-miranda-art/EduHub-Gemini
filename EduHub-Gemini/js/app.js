...
// Função para gerar prompt para questões
function generateQuizPrompt(subject, difficulty, questionsCount, topics, examples) {
        ...
        flashcard.innerHTML = `
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="text" placeholder="Pergunta ou termo" class="w-full px-3 py-2 border border-gray-300 rounded-lg">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Verso</label>
                    <input type="text" placeholder="Resposta ou definição" class="w-full px-3 py-2 border border-gray-300 rounded-lg">
                </div>
            </div>
...
function handleFlashcardSubmit(e) {
    ...
    if (title && subject && flashcards.length > 0) {
        ...
        setTimeout(() => {
            ...
            // Manter apenas um flashcard
            while (elements.flashcardList.children.length > 1) {
                elements.flashcardList.removeChild(elements.flashcardList.lastChild);
            }
        }, 800);
    } else {
        showToast('Por favor, preencha todos os campos e adicione pelo menos um cartão', 'error');
    }
}

// Lidar com submissão de quiz com integração da API Gemini
async function handleQuizSubmit(e) {
    ...
    const questionsCount = formData.get('questionsCount');
    const topics = formData.get('topics');
    const examples = formData.get('examples');
    const visibility = formData.get('quizVisibility');
    
    if (title && subject && questionsCount) {
        // Mostrar estado de carregamento
        const submitBtn = elements.quizForm.querySelector('button[type="submit"]');
        const submitText = submitBtn.querySelector('.submit-text');
        const originalText = submitText.textContent;
        
        submitText.innerHTML = '<div class="spinner"></div>Gerando com IA...';
        submitBtn.disabled = true;
        
        try {
            // Gerar prompt para a API Gemini
            const prompt = generateQuizPrompt(subject, difficulty, questionsCount, topics, examples);
            ...
            if (questions.length > 0) {
                // Salvar quiz gerado (em uma aplicação real, seria salvo no backend)
                const quiz = {
                    id: Date.now(),
                    title,
                    subject,
                ...
                const savedQuizzes = JSON.parse(localStorage.getItem('eduHubQuizzes') || '[]');
                savedQuizzes.push(quiz);
                localStorage.setItem('eduHubQuizzes', JSON.stringify(savedQuizzes));
                
                // Atualizar estatísticas do usuário
                mockData.user.stats.quizzesSolved++;
                mockData.user.xp += parseInt(questionsCount) * 15;
                ...
                saveUserDataToStorage();
                hideAllModals();
                showToast(`Simulado "${title}" com ${questions.length} questões gerado com sucesso!`, 'success');
                
                // Mostrar preview das questões geradas
                setTimeout(() => {
            ...
            showToast('Erro ao gerar o simulado. Verifique sua conexão e tente novamente.', 'error');
        } finally {
            // Restaurar botão
